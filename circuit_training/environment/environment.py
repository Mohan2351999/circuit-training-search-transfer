...
class CircuitEnv(object):
  ...
  def reset(self) -> ObsType:
    ...
    self._done = False
    self._current_mask = self._get_mask()
    # print(f"^^^^^^^^^^^ From the environment file reset function {self._current_node} and {self._done} and {self._get_obs().keys()}")
  ...
  def step(self, action: int) -> Tuple[ObsType, float, bool, Any]:
    ...
    # print(f"^^^^^^^^^^^^^ In the environment step function, current node {self._current_node} and total hard macros: {self._num_hard_macros}")
    if not self._done and not np.any(self._current_mask):
      logging.info('before becoming infeasible: %s',
                   self._current_actions)
      info = {
          'wirelength': -1.0,
          'congestion': -1.0,
    ...
    # print("^^^^^^^^^^^^^ In the environment step function called anlytical placer")
    cost, info = self.call_analytical_placer_and_get_cost()
    # print(f"$$$$$$$$$$$$$$ The current info {info}")
    # print(f"^^^^^^^^^^^^ cost {cost}, is done: {self._done}, \n and the info : {info}")
    # print(f"^^^^^^^^^^^^ The values inside the environemnt step function {self._get_obs()}, {cost}, {self._done}, {info}")
